...
请以专业配音导演的身份，用专业术语和实用建议来回答。""",
            ...
            'scene_dialogue_generation': """你是一位资深的专业编剧，拥有丰富的影视剧本创作经验。你擅长创作自然流畅的对话，能够通过台词展现角色性格、推进剧情发展，并营造恰当的戏剧张力。
...
请以资深制作顾问的身份，提供全面而实用的指导建议。"""
    ...
    def get_voice_design_prompt(self, character_info: str, scene_context: str = "", emotional_state: str = "") -> str:
        """获取角色音色设计提示词"""
        try:
    ...
    def get_dialogue_generation_prompt(self, scene_description: str, characters: str, story_context: str, emotional_tone: str = "", scene_duration: str = "") -> str:
        """获取台词生成提示词"""
        try:
    ...
    def get_audio_design_prompt(self, scene_description: str, time_setting: str = "", location: str = "", weather: str = "", mood: str = "", duration: str = "") -> str:
        """获取音效设计提示词"""
        try:
    ...
    def get_art_style_prompt(self, project_type: str, story_theme: str, target_audience: str = "", emotional_tone: str = "", time_period: str = "", location_style: str = "") -> str:
        """获取美术风格提示词"""
        try:
    ...
    def get_comprehensive_guidance_prompt(self, project_overview: str, budget_level: str = "", production_timeline: str = "", team_size: str = "", technical_resources: str = "") -> str:
        """获取综合创作指导提示词"""
        try:
    ...
    def validate_prompt_parameters(self, prompt_type: str, parameters: Dict[str, Any]) -> bool:
        """验证提示词参数的完整性"""
        required_params = {
            'character_voice_design': ['character_info'],
            'scene_dialogue_generation': ['scene_description', 'characters', 'story_context'],
        ...
        for param in required_params[prompt_type]:
            if param not in parameters or not parameters[param]:
                self.logger.warning(f"Missing required parameter '{param}' for prompt type '{prompt_type}'")
                return False
