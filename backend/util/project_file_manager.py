...
class ProjectFileManager:
    ...
    def _get_feature_directory(self, project_name: str, feature_type: str) -> str:
        ...
        return os.path.join(
            self._get_project_path(project_name),
            self.directories[feature_type]
        )
    ...
    # 角色音色配置管理
    def save_character_voice_profile(self, project_name: str, character_id: str, voice_profile: Dict[str, Any]) -> bool:
        """保存角色音色配置"""
        try:
    ...
    def load_character_voice_profile(self, project_name: str, character_id: str) -> Optional[Dict[str, Any]]:
        """加载角色音色配置"""
        try:
            directory = self._get_feature_directory(project_name, 'character_voices')
            file_path = os.path.join(directory, f"{character_id}_voice_profile.json")
    ...
    # 分镜台词管理
    def save_scene_dialogue(self, project_name: str, scene_id: str, dialogue: Dict[str, Any]) -> bool:
        """保存分镜台词"""
        try:
    ...
    def load_scene_dialogue(self, project_name: str, scene_id: str) -> Optional[Dict[str, Any]]:
        """加载分镜台词"""
        try:
            directory = self._get_feature_directory(project_name, 'scene_dialogues')
            file_path = os.path.join(directory, f"{scene_id}_dialogue.json")
    ...
    # 环境音效设计管理
    def save_audio_design(self, project_name: str, scene_id: str, audio_design: Dict[str, Any]) -> bool:
        """保存音效设计"""
        try:
    ...
    def load_audio_design(self, project_name: str, scene_id: str) -> Optional[Dict[str, Any]]:
        """加载音效设计"""
        try:
            directory = self._get_feature_directory(project_name, 'scene_audio')
            file_path = os.path.join(directory, f"{scene_id}_audio_design.json")
    ...
    # 美术风格指南管理
    def save_art_style_guide(self, project_name: str, art_style_guide: Dict[str, Any]) -> bool:
        """保存美术风格指南"""
        try:
    ...
    def create_project_structure(self, project_name: str) -> bool:
        ...
        try:
            ...
            self._save_json_file(info_file, project_info)
        ...
        try:
            ...
            for item in os.listdir(self.base_path):
                ...
                if os.path.isdir(item_path):
                    # 检查是否有项目信息文件
                    info_file = os.path.join(item_path, 'project_info.json')
    ...
    def get_project_info(self, project_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return self._load_json_file(info_file)
    ...
    def import_project_data(self, import_path: str, project_name: str = None) -> bool:
        ...
        try:
            ...
            # 如果项目已存在，创建备份
            if os.path.exists(project_path):
                backup_path = project_path + '_backup_' + datetime.now().strftime('%Y%m%d_%H%M%S')
