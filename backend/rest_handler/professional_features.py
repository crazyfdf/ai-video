...
def generate_scene_dialogue():
    ...
    try:
        ...
        # 验证必需字段
        error = handler._validate_request_data(
            ["sceneDescription", "characters", "storyContext"], data
        )
        if error:
            return jsonify({"error": error}), 400
        ...
        character_voices_data = data.get("characterVoices", [])
        
        # 构建角色音色信息字符串
        character_voices_info = ""
        if character_voices_data:
            voice_descriptions = []
            for char_voice in character_voices_data:
                char_name = char_voice.get("name", "")
                voice_desc = char_voice.get("voiceDescription", "")
                voice_file = char_voice.get("voiceFileName", "")
                
                voice_info = f"角色：{char_name}"
                if voice_desc:
                    voice_info += f"，音色描述：{voice_desc}"
                if voice_file:
                    voice_info += f"，已上传音色文件：{voice_file}"
                voice_descriptions.append(voice_info)
            character_voices_info = "\n".join(voice_descriptions)

        # 生成提示词
        prompt = handler.prompt_engine.get_dialogue_generation_prompt(
            scene_description, characters, story_context, emotional_tone, scene_duration, character_voices_info
        )
...
def load_scene_dialogue():
    ...
    try:
        ...
        # 加载台词
        dialogue_data = handler.file_manager.load_scene_dialogue(project_name, scene_id)
...
def load_environment_audio_design():
    ...
    try:
        ...
        # 加载音效设计
        audio_data = handler.file_manager.load_audio_design(project_name, scene_id)
        ...
        return jsonify(
            {
                ...
                "voiceSettings": {"voice": voice, "rate": rate, "pitch": pitch},
            }
        )
...
def list_professional_features():
    ...
    try:
        ...
        result = {
            ...
            "characterVoices": handler.file_manager.list_character_voice_profiles(
                project_name
            ),
            ...
            "hasArtStyleGuide": handler.file_manager.load_art_style_guide(project_name)
            is not None,
        }
