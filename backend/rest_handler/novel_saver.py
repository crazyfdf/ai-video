...
def save_novel_to_project():
    ...
    try:
        ...
        # 创建项目专用的novels目录
        project_novels_dir = get_project_dir(project_name, "novels")
...
def save_prompt_to_project():
    ...
    try:
        ...
        # 创建项目专用的prompts目录
        project_prompts_dir = get_project_dir(project_name, "prompts")
...
def load_project_novels(project_name=None):
    """加载项目的所有小说"""
    try:
        project_novels_dir = get_project_dir(project_name, "novels")
        index_path = os.path.join(project_novels_dir, "novel_index.json")
        ...
        logging.info(
            f"Novels loaded successfully from project {project_name}: {len(novel_index)} novels"
        )
...
def load_project_prompts(project_name=None):
    ...
    try:
        ...
        logging.info(
            f"Prompts loaded successfully from project {project_name}: {len(prompt_index)} prompts"
        )
